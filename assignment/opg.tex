...
\begin{proposition}
	For $x_1, ..., x_k \in \mbb Z$ we get a decomposable form $F(x_1, ..., x_k) = N_{K / \mbb Q}(x_1 \mu_1 + ... + x_n \mu_k)$ of degree $n$. % Furthermore, there is a subfield $L$ of $\mbb C$ which is an extension of $K$ and is Galois over $\mbb Q$, so that $F$ dec there exists a Galois extension $L$ containing $K$ over which $F$ is decomposable.  
\end{proposition}
\begin{proof}
...
%Thus, acting with $\sigma$ on \cref{eq: coefficient} we get
\begin{align*}
...
%from $\{1, ..., n\}$ to itself. The consider the set of all $\tau'$ 	and the coefficient of $x_\tau(1) ... x_\tau(n)$  will be sums $\sum_{i=1}^n \sigma_i(\mu_\tau(i))$, with $\tau$ being any function from $\{1, ..., n\}$ to itself. Since the norm, $N_{K/\mbb Q}$, sends elements in $K$ to $\mbb Q$ we know that $F(x_1, ..., x_n)$ is in $\mbb Q$. Thus it is fixed by every single $\sigma \in G$. This implies that all the coefficients of $F$ are fixed by every $\sigma \in G$, so they all belong to $\mbb Q$. Therefore $F$ is a form of degree $n$.
\begin{definition}
...
%for $K$ as a vector space over $\mbb Q$ which is also a basis for $M$. (MAYBE JUSTIFY THIS)
...
\end{proposition}
% \begin{proof}
% 	First of all it is clear that (2.) implies (1.). To see that (1.) implies (3.) suppose that $\abs{x}_1 < 1$. Then $x^n$ converges to $0$ with respect to $\abs{}_1$ and as $\abs{}_1$ induce the same topologies on $K$ we must have that $x^n$ converges to $0$ with respect to $\abs{}_2$ so $\abs{x}_2 < 1$. It remains to show that (3.) implies (2.). Assume that $y \in K$ so that $\abs{y}_1 > 1$ and that $x \in K^*$. Then there is $\alpha \in \mbb R$ so that $\abs{x}_1 = \abs{y}_1^\alpha$.
% \end{proof}
...
%There are for example many different isomorphisms between isomorphic vector spaces.
...
\end{proof}
% \begin{remark}
% 	Because $\abs{}'$ extends $\abs{}$, we will, for ease of notation, often denote $\abs{}'$ by $\abs{}$.
% \end{remark}
...
\begin{proof}
	% This theorem holds in general but we will only prove it for the case where $\abs{}$ is non-archimedian. So suppose that $\abs{}$ is archimedian and that $n$ above is finite.
...
\begin{definition}
	A local field of characteristic 0 is called a $\mfrak p$-adic number field. % where $\mfrak p$ refers to the unique maximal ideal of the valuation ring of $K$.
\end{definition}
...
% If it were possible to give a version of \cref{lem: Two power series are equal} requiring that $n_1 + n_2  \geq n$ rather than $n_1 = n - 2$ and $n_2 \geq 2$,   
