...
\begin{lemma} \label{lem: Two power series are equal}
    ...
    \end{align*}
    with the $a_{ij}$ and $b_{ij}$ in $K$ and where the both groups of equations are linearly independent (WHAT DOES THIS REALLY MEAN IN THIS CONTEXT?). Suppose that there exists a solution, $P_1(t), ..., P_n(t)$, in formal power series with coefficients in $K$, and with zero constant term. Then there are two indices $k \neq l$ so that $P_k(t) = P_l(t)$.
\end{lemma}
